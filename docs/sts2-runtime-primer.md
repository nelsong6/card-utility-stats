...
One asymmetry to keep in mind when extending this. A restarted **combat** costs the run record nothing, because combat stats live in `_pendingCombat` until promotion and the adopt path drops that buffer. A restarted **shop or event** does not get that for free: gold spent, cards bought, relics taken and HP traded are committed as they happen, so the rollback leaves them in the run file as history. `AdoptRunLocked` keeps the deck view coherent — it re-binds instance numbers from the live deck and drops rank restores it can no longer match — which is the same tolerance the tracker already needed for a crash or save-and-quit rewinding past a card acquisition. Undoing the *record* as well would require a SpireLens-side room snapshot, which is exactly the machinery this feature exists without.
