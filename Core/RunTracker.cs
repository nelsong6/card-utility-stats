...
public static class RunTracker
{
    ...
    /// <summary>
    /// Record Vigor gained from Akabeko's combat-start effect.
    /// <paramref name="amount"/> is the Vigor granted (normally 8).
    /// Called from <see cref="Patches.AkabeloBeforeSideTurnStartPatch"/>.
    /// </summary>
    public static void RecordAkabeloVigorGained(int amount)
    {
        if (amount <= 0) return;
        ...
        lock (_lock)
        {
            ...
            {
                _pendingCombat ??= new PendingCombat();
                if (!_pendingCombat.RelicAggregates.TryGetValue(AkabeloRelicId, out var agg))
                {
                    agg = new RelicAggregate();
                    _pendingCombat.RelicAggregates[AkabeloRelicId] = agg;
                }
                agg.VigorGained += amount;
            }
            catch (Exception e)
            {
                CoreMain.LogDebug($"RecordAkabeloVigorGained failed: {e.Message}");
            }
        }
    }
