...
/// <summary>
/// Records Nutritious Oyster's owner-specific max-HP gain by observing the
/// owner's actual max HP after the async pickup effect finishes.
/// </summary>
[HarmonyPatch(typeof(NutritiousOyster), nameof(NutritiousOyster.AfterObtained))]
public static class NutritiousOysterAfterObtainedPatch
{
    [HarmonyPrefix]
    public static void Prefix(NutritiousOyster __instance, out MaxHpState __state)
    {
        __state = default;
        ...
        {
            CoreMain.LogDebug($"NutritiousOysterAfterObtainedPatch.Prefix failed: {e.Message}");
        }
    }
    ...
    public static void Postfix(MaxHpState __state, Task __result)
    {
        ...
        {
            CoreMain.LogDebug($"NutritiousOysterAfterObtainedPatch.Postfix failed: {e.Message}");
        }
    }
    ...
    private static void Observe(MaxHpState state)
    {
        try
        {
            ...
            RunTracker.RecordNutritiousOysterMaxHpGained(
                creature,
                maxHpGained,
                state.InitialMaxHp,
                creature.MaxHp);
        ...
        {
            CoreMain.LogDebug($"NutritiousOysterAfterObtainedPatch.Observe failed: {e.Message}");
        }
    }
