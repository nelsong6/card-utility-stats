...
/// <summary>
/// Records Pear's owner-specific max-HP gain by observing the owner's
/// actual max HP after the async pickup effect finishes.
/// </summary>
[HarmonyPatch(typeof(Pear), nameof(Pear.AfterObtained))]
public static class PearAfterObtainedPatch
{
    [HarmonyPrefix]
    public static void Prefix(Pear __instance, out MaxHpState __state)
    {
        __state = default;
        ...
        {
            CoreMain.LogDebug($"PearAfterObtainedPatch.Prefix failed: {e.Message}");
        }
    }
    ...
    public static void Postfix(MaxHpState __state, Task __result)
    {
        ...
        {
            CoreMain.LogDebug($"PearAfterObtainedPatch.Postfix failed: {e.Message}");
        }
    }
    ...
    private static void Observe(MaxHpState state)
    {
        try
        {
            ...
            RunTracker.RecordPearMaxHpGained(creature, maxHpGained, state.InitialMaxHp, creature.MaxHp);
        }
        catch (Exception e)
        {
            CoreMain.LogDebug($"PearAfterObtainedPatch.Observe failed: {e.Message}");
        }
    }
