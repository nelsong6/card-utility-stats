...
/// <summary>
/// Records Mango's owner-specific max-HP gain by observing the owner's actual
/// max HP after the async pickup effect finishes.
/// </summary>
[HarmonyPatch(typeof(Mango), nameof(Mango.AfterObtained))]
public static class MangoAfterObtainedPatch
{
    [HarmonyPrefix]
    public static void Prefix(Mango __instance, out MaxHpState __state)
    {
        __state = default;
        ...
        {
            CoreMain.LogDebug($"MangoAfterObtainedPatch.Prefix failed: {e.Message}");
        }
    }
    ...
    public static void Postfix(MaxHpState __state, Task __result)
    {
        ...
        {
            CoreMain.LogDebug($"MangoAfterObtainedPatch.Postfix failed: {e.Message}");
        }
    }
    ...
    private static void Observe(MaxHpState state)
    {
        try
        {
            ...
            RunTracker.RecordMangoMaxHpGained(
                creature,
                maxHpGained,
                state.InitialMaxHp,
                creature.MaxHp);
        ...
        {
            CoreMain.LogDebug($"MangoAfterObtainedPatch.Observe failed: {e.Message}");
        }
    }
