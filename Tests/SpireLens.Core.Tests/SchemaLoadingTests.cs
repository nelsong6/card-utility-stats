...
public class SchemaLoadingTests
{
    ...
    [Fact]
    public void HistoricalLoad_AcceptsCurrentV17Fixture()
    {
        var loaded = RunStorage.LoadHistorical(FixturePath("v17-anchor-run.json"));
        ...
        var relicAgg = loaded.Data.RelicAggregates["RELIC.ANCHOR"];
        Assert.Equal(30, relicAgg.BlockGained);
    }
    ...
    [Fact]
    public void ResumableLoad_AcceptsCurrentV17Fixture()
    {
        var resumed = RunStorage.LoadResumable(FixturePath("v17-anchor-run.json"));

        Assert.NotNull(resumed);
        Assert.Equal(RunData.CurrentSchemaVersion, resumed!.SchemaVersion);
        var relicAgg = resumed.RelicAggregates["RELIC.ANCHOR"];
        Assert.Equal(30, relicAgg.BlockGained);
    }
