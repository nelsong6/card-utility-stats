...
- `scenario_setup.next_normal_encounter` is OPTIONAL. Default to `null` and omit the corresponding `scenario_id_validation.encounters` entry. next_normal_encounter only selects WHICH Monster (normal) encounter is fought; it can NEVER create an Elite or Boss room — use required_room_type (the next rule) to stage room-type-gated effects. Only specify an encounter id when the behavior depends on which specific normal opponent is fought (e.g. a damage-on-combat-start interaction against a particular enemy). For encounter-agnostic behaviors — start-of-combat triggers like Akabeko's Vigor gain, end-of-combat hooks, persistent passives — leave it null; the base save's existing encounter slot is fine. When you do specify it, validate the id with `list_encounters` (preferred — pick a real id from the returned list) or `lookup_encounter`, never from memory or any "default" advertised in capabilities. The validator rejects ids not present in the live encounter catalog, so a hallucinated or stale id aborts the entire run.
- scenario_setup.required_room_type stages a room-type-gated trigger and is the ONLY correct way to test one. When the behavior under test fires only in a specific room type — relic/card text such as 'at the start of Elite combats' or a Boss-only effect — set required_room_type to that RoomType: 'Monster', 'Elite', or 'Boss'. The harness deterministically enters that room after loading the save and the scenario FAILS unless the live combat is actually that type, so the effect is genuinely exercised. Leave it null for behavior that fires in any combat (most start-of-combat and end-of-combat triggers, persistent passives). Never fake an Elite/Boss by putting an _ELITE/_BOSS encounter id into next_normal_encounter: that produces a normal Monster room and the relic never fires — that is the spirelens#146 failure. required_room_type is not a catalog id and needs no scenario_id_validation entry.
...
- Before changing card or relic attribution hooks, read `AGENTS.md` and `docs/sts2-runtime-primer.md`, then inspect the nearest existing hook pattern in `Core/Patches` and `Core/RunTracker.cs`. STS2 card/relic hooks often participate in async flows; do not arm an attribution flag in a prefix and clear it in the same method's postfix unless source inspection proves the observed outcome happens before that postfix. For async relic/card effects, follow the existing later-boundary cleanup pattern such as Orichalcum's `Hook.AfterTurnEnd` cleanup: keep the attribution window open until the resource/block/draw mutation hook can observe the real outcome, then consume it there and clear any leftover window at a reliable later hook boundary.
- For tooltip-visible card or relic stats, verify the tooltip path reads the same live aggregate that the recorder updates during the current combat. Existing tooltip helpers intentionally merge committed run data with pending combat data so screenshots can prove mid-combat effects before `CombatEnded`; do not implement a stat that is only visible after combat unless the issue explicitly asks for post-combat history.
- For relic tooltip dispatch, prefer a compiled model type or stable relic id over raw `GetType().Name` string checks. If a direct model type is unavailable, inspect the model's id/capabilities and log/handle the resolved catalog id rather than depending on a guessed CLR type name.
- For visible text evidence, render a compact literal label/value that a screenshot OCR can recover, matching the user-facing wording and capitalization from the issue. If the issue/test scenario implies a row like `Energy generated: 1`, the tooltip body must visibly contain that label, colon, and value together; a model-only unit test or hidden aggregate is not enough.
