...
function Get-FileListText {
    ...
    $items = foreach ($file in $files) {
        $relative = $file.FullName.Substring($root.Length).TrimStart([char[]]@([char]'\\', [char]'/'))
        '- `' + $relative + '`'
    }
...
function Add-FailureCategory {
    param([object]$Bucket, [string]$Category)
    if ([string]::IsNullOrWhiteSpace($Category)) { return }
    if (-not $Bucket.failure_categories.Contains($Category)) { $Bucket.failure_categories[$Category] = 0 }
...
function Format-FailureCategories {
    ...
    foreach ($property in $Categories.GetEnumerator()) {
        $items.Add("$($property.Key):$($property.Value)") | Out-Null
    }
    return ($items -join ', ')
}
...
function Get-ClaudeToolCallSummary {
    ...
    $summary = [ordered]@{ total = New-ToolMetricBucket; phases = [ordered]@{} }
...
if ($null -ne $prNumber -and -not [string]::IsNullOrWhiteSpace([string]$prNumber)) {
    Add-GhComment -TargetKind 'pr' -TargetNumber ([string]$prNumber) -Body $markdown
} | ConvertFrom-Json -ErrorAction Stop } catch { return }
        $phaseName = [string](Get-NestedPropertyValue -Object $record -Path @('data', 'phase'))
        if ([string]::IsNullOrWhiteSpace($phaseName) -or -not $summary.phases.Contains($phaseName)) { $phaseName = '_unknown' }
        if (-not $summary.phases.Contains($phaseName)) { $summary.phases[$phaseName] = New-ToolMetricBucket }
        $bucket = $summary.phases[$phaseName]
        $total = $summary.total
        ...
        if ($record.kind -eq 'tool_result') {
            $bucket.tool_results++
            $total.tool_results++
            $failed = [bool](Get-NestedPropertyValue -Object $record -Path @('data', 'failed'))
            $category = [string](Get-NestedPropertyValue -Object $record -Path @('data', 'failure_category'))
            if ($failed) {
                $bucket.failed_tool_results++
                $total.failed_tool_results++
                Add-FailureCategory -Bucket $bucket -Category $category
                Add-FailureCategory -Bucket $total -Category $category
                if ($category -eq 'permission_denied') {
                    $bucket.permission_denials++
                    $total.permission_denials++
                }
            }
        }
    }
