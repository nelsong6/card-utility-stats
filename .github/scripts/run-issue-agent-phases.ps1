...
- `scenario_setup` is the deterministic pre-verification save recipe. Choose the correct `base_save_name` from the verified character identity, such as `base_regent`, `base_ironclad`, `base_silent`, `base_defect`, or `base_necrobinder`. Use a complete small `deck` of real card ids that lets normal gameplay reach the evidence state quickly; for Make It So, include `MAKE_IT_SO` plus three real Regent Skill cards and enough energy/max_energy to play them. Do not tell verification to use dev-console `fight`/`card` commands for card availability; card availability must come from this save recipe.
