...
- Keep repository and dependency discovery scoped to this run. Do not search above `$RepoRoot`, under sibling or parent runner workspaces, or inside stale `issue-agent-src` directories from older runs. When you need game assembly metadata, use `Sts2PathDiscovery.props`, the configured `Sts2DataDir`, or a focused build in the current checkout to materialize publicized assemblies under the current `$RepoRoot`. If those paths are unavailable, abort with `missing_code_context` instead of scanning `C:\` or reusing old runner artifacts.
